...
public class DemoHelper {
    ...
    private void InitCallKit(Context context) {
        ...
        //设置声网AgoraAppId
        callKitConfig.setAgoraAppId("4afc28b5918f43cab78b9ddb01032104");
        callKitConfig.setEnableRTCToken(false);
    ...
    /**
     * 检查是否是第一次安装登录 默认值是true, 需要在用api拉取完会话列表后，就其置为false.
     */
    ...
    /**
     * 将状态置为非第一次安装，在调用获取会话列表的api后调用 并将会话列表是否来自服务器置为true
     */
